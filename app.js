...
// ═══════════ 消費税 税区分 체계 (소비세법) ═══════════
var TAXCLS_BUY  = ['課税仕入10%','課税仕入8%','非課税','不課税','対象外'];
var TAXCLS_SELL = ['課税売上10%','課税売上8%','輸出免税0%','非課税売上','不課税売上'];
var TAXCLS_ALL  = TAXCLS_BUY.concat(TAXCLS_SELL);
// 個別対応方式 用途区分 (세리사 채택 시 사용)
var TAXUSE = ['課税売上対応','非課税売上対応','共通対応'];
// 税区分 → 세율
var _RN=['Ⅰ','Ⅱ','Ⅲ','Ⅳ','Ⅴ','Ⅵ','Ⅶ','Ⅷ'];
function _rn(){ window._plN=(window._plN||0)+1; return _RN[window._plN-1]||('('+window._plN+')'); }
function taxClsOptions(sel){
  var v=sel||'';
  var o='<option value="">-</option>';
  o+='<optgroup label="── 매입 (仕入) ──">';
  TAXCLS_BUY.forEach(function(c){o+='<option value="'+c+'"'+(c===v?' selected':'')+'>'+c+'</option>';});
  o+='</optgroup><optgroup label="── 매출 (売上) ──">';
  TAXCLS_SELL.forEach(function(c){o+='<option value="'+c+'"'+(c===v?' selected':'')+'>'+c+'</option>';});
  o+='</optgroup>';
  return o;
}
// 계정 선택 시 税区分 자동 설정
function autoTaxCls(tr){
  if(!tr) return;
  var ac=tr.querySelector('.sl_acct'), tc=tr.querySelector('.sl_taxcls');
  if(!ac||!tc||!ac.value) return;
  if(tc.value) return;                       // 사용자가 이미 고른 값은 유지
  var def=ACCT_TAX_DEFAULT[ac.value];
  if(def) tc.value=def;
}
function taxRateOf(cls){
  if(cls==='課税仕入10%'||cls==='課税売上10%') return 10;
  if(cls==='課税仕入8%'||cls==='課税売上8%') return 8;
  return 0;
}
// 계정별 기본 税区分 (전표 입력 시 자동 선택 · 마이그레이션 기준)
var ACCT_TAX_DEFAULT = {
  // ── 매출 ──
  '400':'課税売上10%','415':'輸出免税0%','416':'不課税売上','417':'課税売上10%',
  '401':'非課税売上','402':'不課税売上','403':'対象外','404':'対象外','405':'不課税売上',
  '406':'課税売上10%','407':'課税売上10%','408':'非課税売上','409':'課税売上10%',
  '410':'課税仕入10%','411':'対象外','412':'対象外','413':'対象外','414':'対象外',
  // ── 매출원가 ──
  '500':'課税仕入10%','501':'課税仕入10%','502':'課税仕入10%','503':'対象外','504':'課税仕入10%',
  // ── 인건비 (급여·보수는 불과세이나 실무상 対象外) ──
  '510':'対象外','511':'対象外','512':'対象外','513':'対象外','515':'対象外','514':'課税仕入10%',
  // ── 판관비 ──
  '516':'課税仕入10%','520':'課税仕入10%','521':'不課税','522':'課税仕入10%','523':'課税仕入10%',
  '524':'課税仕入10%','525':'課税仕入10%','526':'課税仕入10%','527':'課税仕入10%','528':'課税仕入10%',
  '529':'対象外','530':'対象外','531':'課税仕入10%','532':'課税仕入10%','533':'課税仕入10%',
  '534':'課税仕入10%','535':'課税仕入10%','536':'課税仕入10%','537':'課税仕入10%',
  '538':'課税仕入10%','539':'課税仕入10%','547':'課税仕入10%','548':'課税仕入10%','549':'課税仕入10%',
  '570':'課税仕入10%','580':'課税仕入10%','581':'課税仕入10%','582':'課税仕入10%','583':'課税仕入10%','584':'課税仕入10%',
  // ── 영업외·법인세 ──
...
var ACCT_EXTRA = [
  ...
  {c:'416',n:'売上高-国外不課税',k:'매출-국외불과세',g:'수익'},
  {c:'417',n:'売上値引・返品',k:'매출할인·환입',g:'수익'}
];
function mergeAccts(){
  var base=ACCT_INIT.concat(ACCT_EXTRA||[]).concat(D.customAccts||[]).filter(function(a){return a&&a.c;});
  var seen={},out=[];
  base.forEach(function(a){ if(!seen[a.c]){seen[a.c]=1;out.push(a);} });
  return out.sort(function(x,y){return x.c.localeCompare(y.c);});
}
...
function dynamicFS(){
  ...
  const su=suT;
  // ── Ⅰ 売上高 (400 국내 + 415 수출면세 + 416 국외불과세 + 417 값인·반품(음수)) ──
  const salesCodes=['400','415','416','417'];
  let sales=0; const salesDetail=[];
  salesCodes.forEach(function(c2){var v=acctBalFY(c2);sales+=v;if(v!==0)salesDetail.push({c:c2,v:v});});
  // ── Ⅱ 売上原価 = 期首棚卸 + 当期仕入 − 期末棚卸 ──
  const invCodes=['140','141','142','143','144','145'];
  const _fyN=curFY();
  const _prevEnd=_fyN>1?fyRange(_fyN-1).end:null;
  const _thisEnd=fyRange(_fyN).end;
  let invBegin=0,invEnd=0;
  invCodes.forEach(function(c2){
    if(_prevEnd) invBegin+=acctBalAsOf(c2,_prevEnd);
    invEnd+=acctBalAsOf(c2,_thisEnd);
  });
  const cogsCodes=['500','501','502','503','504'];
  let purchase=0; const cogsDetail=[];
  cogsCodes.forEach(function(c2){var v=acctBalFY(c2);purchase+=v;if(v!==0)cogsDetail.push({c:c2,v:v});});
  const cogs=invBegin+purchase-invEnd;
  const gross=sales-cogs;                  // 売上総利益
  const opInc=gross-sgaT-su;               // 営業利益 (기존 ol과 동일 산식, 매출 반영)
  const ol=opInc;                          // 하위호환
  ...
  const deposit=acctBalAsOf('110',_bsEnd);
  const secDep=acctBalAsOf('191',_bsEnd);
  const secBookVal=acctBalAsOf('130',_bsEnd); // 전표 장부가
  const fxDep=acctBalAsOf('192',_bsEnd); // 외화증권예수금 (예수금 → 현금성자산)
  const secMV=c.allMv;
  ...
  const cashT=deposit+secDep+fxDep;
  // Other assets (fixed assets, prepaid, etc.) — all asset accounts except 110, 191, 130
  let otherAssets=0;
  D.accts.filter(function(ac){return ac.g==='자산'&&['110','191','130','192'].indexOf(ac.c)<0;}).forEach(function(ac){otherAssets+=acctBalAsOf(ac.c,_bsEnd);});
  const totA=cashT+secForBS+otherAssets;
  // Liabilities + Equity: all from journals
  let totL=0;D.accts.filter(function(ac){return ac.g==='부채';}).forEach(function(ac){totL+=acctBalAsOf(ac.c,_bsEnd);});
  // Equity: from journals
  const capitalBal=acctBalAsOf('300',_bsEnd)+acctBalAsOf('301',_bsEnd)+acctBalAsOf('302',_bsEnd);
  const retainedBal=acctBalAsOf('310',_bsEnd)+acctBalAsOf('311',_bsEnd)+acctBalAsOf('312',_bsEnd)+retainedCarry(); // +전기이월
...
// ===== ASSET TREND (자산추이) =====
function rSnapList(){
  var sn=(D.snapshots||[]).slice().sort(function(a,b){return b.dt.localeCompare(a.dt);});
  if(!sn.length) return '<div style="font-size:11px;color:#94a3b8;padding:8px">스냅샷이 없습니다.</div>';
  var h='<table style="font-size:11px"><thead><tr><th>일자</th><th class="r">총자산</th><th class="r">법인계좌</th><th class="r">증권계좌</th><th>회기</th><th></th></tr></thead><tbody>';
  sn.forEach(function(x,i){
    h+='<tr class="'+(i%2?'a':'')+'"><td>'+x.dt+'</td><td class="r m">'+fm(x.totA)+'</td><td class="r m">'+fm(x.bb)+'</td><td class="r m">'+fm(x.secBal)+'</td>'+
       '<td>'+(x.fy?('제'+x.fy+'기'):'<span style="color:#d97706">미기록</span>')+'</td>'+
       '<td><button class="del" onclick="delSnapshot(\''+x.dt+'\')">✕</button></td></tr>';
  });
  h+='</tbody></table>';
  h+='<div style="display:flex;gap:6px;margin-top:8px;align-items:center;flex-wrap:wrap">'+
     '<input id="snapCut" type="date" style="padding:4px 6px;border:1px solid #e2e6ed;border-radius:4px;font-size:11px">'+
     '<button class="bt rd" style="font-size:11px" onclick="delSnapshotsBefore()">지정일 이전 일괄 삭제</button>'+
     '<button class="bt gh" style="font-size:11px" onclick="refreshSnapshot()">오늘 스냅샷 재계산</button></div>';
  return h;
}
function delSnapshot(dt){
  if(!confirm(dt+' 스냅샷을 삭제하시겠습니까?'))return;
  D.snapshots=(D.snapshots||[]).filter(function(s){return s.dt!==dt;});
  saveD();
  var el=document.getElementById('snapMgr'); if(el)el.innerHTML=rSnapList();
  toast(dt+' 스냅샷 삭제','info');
}
function delSnapshotsBefore(){
  var el=document.getElementById('snapCut'); var cut=el?el.value:'';
  if(!cut)return toast('기준일을 선택하세요','warn');
  var before=(D.snapshots||[]).length;
  var tgt=(D.snapshots||[]).filter(function(s){return s.dt<cut;}).length;
  if(!tgt)return toast('삭제 대상이 없습니다','info');
  if(!confirm(cut+' 이전 스냅샷 '+tgt+'건을 삭제하시겠습니까?'))return;
  D.snapshots=(D.snapshots||[]).filter(function(s){return s.dt>=cut;});
  saveD();
  var m=document.getElementById('snapMgr'); if(m)m.innerHTML=rSnapList();
  toast('스냅샷 '+(before-D.snapshots.length)+'건 삭제','ok');
}
function refreshSnapshot(){
  saveSnapshot();
  var m=document.getElementById('snapMgr'); if(m)m.innerHTML=rSnapList();
  toast('오늘 스냅샷을 현재 값으로 재계산했습니다','ok');
}
function saveSnapshot(){
  ...
  const snap={dt:today,totA:c.totA,bb:c.bb,secBal:c.secBal,secDep:c.secDep,allMv:c.allMv,allPl:c.allPl,rpl:c.rpl,fxJpy:c.fxJpy||0,otherA:c.totA-c.bb-c.secBal,fy:curFY()};
...
function renderTrendChart(period){
  ...
  // 구형 스냅샷(fxJpy·otherA 미기록) 역산 보정 → 구성 합계 = 총자산 보장
  const _fx=(last.fxJpy!=null)?last.fxJpy:Math.max(0,(last.secBal||0)-(last.secDep||0)-(last.allMv||0));
  const _oa=(last.otherA!=null)?last.otherA:Math.max(0,(last.totA||0)-(last.bb||0)-(last.secBal||0));
  const parts=[
    {label:'법인계좌',val:last.bb,color:'#d97706'},
    {label:'증권예수금',val:last.secDep,color:'#7c3aed'},
    {label:'외화예수금',val:_fx,color:'#0891b2'},
    {label:'유가증권',val:last.allMv,color:'#2563eb'},
    {label:'기타자산',val:_oa,color:'#94a3b8'}
  ].filter(p=>p.val>0);
  ...
  bars+='<span style="color:#64748b">합계 '+fm(barTotal)+(barTotal!==last.totA?' <b style="color:#dc2626">(총자산 '+fm(last.totA)+'과 불일치)</b>':'')+'</span>';
  bars+='</div>';
...
function updateTaxJournal(){
  ...
  const currentTax=acctBalFY('550');
...
function genSlipNo(edt,drCode,crCode){
  ...
  D.journals.forEach(function(j){
    if(j.no&&j.no.indexOf(prefix)===0){var n2=parseInt(j.no.slice(prefix.length),10);if(n2>mx)mx=n2;}
  });
...
const fys=n=>n==null?"-":(n<0?"-¥":"+¥")+fm(Math.abs(n)); // 부호 있는 금액 (실현손익 등)
...
function isCurFYOpen(){ // 선택 회기가 아직 진행 중인가 (과거 회기면 false)
  var t=new Date().toISOString().slice(0,10);
  return curFY()>=fyOf(t);
}
function warekiDate(iso){ // ISO → 令和N年M月D日
...
function fyWareki(fy){ // 회기 → 일본 和暦 기간 문자열
  ...
  // 제1기는 설립일(2025-06-02) 기준 — 신고서 기재와 일치
  var st=(f===1)?'令和7年6月2日（設立日）':w(r.start);
  return '自 '+st+' 至 '+w(r.end);
}
function fySelector(){
  var c=curFY();
...
function exportWord(){
  ...
  let realRows='';
  var _WR=scopeFilter(D.real).sort(function(a,b){return String(a.dt||'').localeCompare(String(b.dt||''));});
  ...
  let bkInRows='',cI=0;
  var _WBI=scopeFilter(D.bkIn);
  var _WBO=scopeFilter(D.bkOut);
...
<body>
<div style="text-align:center;margin-bottom:16pt"><div style="font-size:20pt;font-weight:bold;color:#1e3a5f">태성㈜ 자금운용보고서</div><div style="font-size:11pt;color:#666;margin-top:4pt">${rptDt()} 기준 · ${recScope()==='all'?'전기간 누적 (제1기~제'+curFY()+'기)':'제'+curFY()+'기 '+fyRange(curFY()).start+' ~ '+fyRange(curFY()).end}</div></div>
...
<tr><td style="${S}">자본금</td><td style="${HR}">${fm(acctBalAsOf("300",fyRange(curFY()).end))}</td><td style="${S}"></td></tr>
...
// 税込経理: 세포함 총액 그대로 1건 기표 (154/211 자동분리 없음 — 세리사 방식과 통일)
var newJ={id:_newSlipId,dt,no,desc,dr:d.ac,cr:c.ac,amt:amt,edt,pdt,cur,exp:d.exp||c.exp,vendor:vendor,taxCls:tC,fy:fyOf(edt)};D.journals.push(newJ);saveUndo('create',newJ);
}});});saveD();
// 영향받은 계정 표시
var impactMsg='';
if(!_isEdit){
...
function rSlip(){
  ...
  D.journals.forEach(j=>{
    ...
    var _clsM=fyRange(curFY()).end.slice(0,7);if(fd.startsWith(_clsM)&&j.dt&&j.dt.includes('5/31'))mk=_clsM.replace('-','/')+'(결산)';
  ...
  '<div class="pn" style="padding:14px">'+
    ...
    '<tbody id="slipRows">'+
      ...
      '<td><select class="sl_acct" style="padding:3px;border:1px solid #e2e6ed;border-radius:4px;font-size:11px;width:100%" onchange="slipAutoFill(this.closest(&quot;tr&quot;));autoTaxCls(this.closest(&quot;tr&quot;))">'+acctOptions()+'</select></td>'+
      ...
      '<td><select class="sl_taxcls" style="padding:3px;border:1px solid #e2e6ed;border-radius:4px;font-size:10px">'+taxClsOptions()+'</select></td>'+
...
function rGL(){
  ...
  if(_FJ.length===0){
    return `<div style="display:flex;justify-content:space-between;align-items:center"><div style="display:flex;align-items:center;gap:10px"><div class="pt">총계정원장</div>${fySelector()}</div><button class="bt" onclick="exportGLExcel()" style="background:#059669;font-size:11px">📥 엑셀 내보내기 (日本語)</button></div>
...
function rJrn(){
  ...
  const yms=new Set();
  _FJ.forEach(j=>{ yms.add(jFullDate(j).slice(0,7).replace('-','/')); });
  if(_FJ.some(j=>j.dt.includes('5/31')))yms.add(fyRange(curFY()).end.slice(0,7).replace('-','/'));
  const sortedYMs=[...yms].sort();

  // Build year-month selector
  const ymOpts=sortedYMs.map(ym=>'<option value="'+ym+'">'+ym+'</option>').join('');

  return '<div style="display:flex;justify-content:space-between;align-items:center"><div style="display:flex;align-items:center;gap:10px"><div class="pt">전표조회</div>'+fySelector()+'</div><div style="display:flex;gap:6px"><button class="bt gh" onclick="document.getElementById(\'vendorSummary\').classList.toggle(\'hidden\')" style="font-size:11px">👤 거래처별 집계</button><button class="bt gh" onclick="doUndo()" style="font-size:11px">↩ 되돌리기</button></div></div>'+
...
function rSec(){const c=calc();const jpT=c.jpMv;
  ...
  <div class="pn"><div class="ph"><span>가) 일본</span><button class="bt" onclick="addHoldJP()">+ 종목추가</button></div><div style="overflow-x:auto"><table style="min-width:900px">
    ...
    <tbody>${D.holdJP.map((h,i)=>{const pl=h.mv-h.tc,rr=h.tc?(pl/h.tc*100):0;return`<tr class="${i%2?'a':''}"><td class="b bl">${h.tk}</td><td>${h.nm}</td><td class="r m">${fm(h.sh)}</td><td class="r m">${fm(h.buyAmt)}</td><td class="r m mu">${fm(h.fee)}</td><td class="r m b">${fm(h.tc)}</td><td class="r m">${fm(h.bep)}</td><td class="r m">${h.cp?fm(h.cp):'-'}</td><td class="r m b">${fm(h.mv)}</td><td class="r">${bg(pl)}${(function(){var b=brokerPl(h);if(b===null||b===pl)return '';return '<div style="font-size:9px;color:#94a3b8;margin-top:1px" title="증권사 표시 평균단가 '+h.avgDisp+' 기준">증권사 '+fm(b)+'</div>';})()}</td><td class="r m ${pl>=0?'gn':'rd'}">${rr.toFixed(2)}%</td><td><button class="del" onclick="editHoldJP(${h.id})" style="color:#2563eb;margin-right:4px">✏️</button><button class="del" onclick="delHoldJP(${h.id})">✕</button></td></tr>`;}).join('')}</tbody>
    <tr class="t"><td colspan="3" class="r">합계</td><td class="r m">${fm(D.holdJP.reduce((s,h)=>s+h.buyAmt,0))}</td><td class="r m">${fm(D.holdJP.reduce((s,h)=>s+h.fee,0))}</td><td class="r m">${fm(c.jpC)}</td><td colspan="2"></td><td class="r m">${fm(c.jpMv)}</td><td class="r">${bg(c.jpMv-c.jpC)}${(function(){var b=brokerPlTotal();if(b===null||b===(c.jpMv-c.jpC))return '';return '<div style="font-size:9px;color:#94a3b8;margin-top:1px">증권사 '+fm(b)+'</div>';})()}</td><td class="r m">${((c.jpMv-c.jpC)/c.jpC*100).toFixed(2)}%</td><td></td></tr>
  </table></div></div>
  ...
  <div class="pn"><div class="ph"><span>나-2) 외화예수금 (米国株購入用)</span><button class="bt" onclick="editFxDeposit()" style="font-size:11px">✏️ 편집</button></div>${fxUsdAmt>0?'<table><thead><tr><th>통화</th><th class="r">잔고</th><th class="r">취득환율</th><th class="r">취득금액(JPY)</th><th class="r">평가환율</th><th class="r">평가금액(JPY)</th><th class="r">평가손익</th></tr></thead><tbody><tr><td class="b bl">USD 米ドル</td><td class="r m">$'+fxUsdAmt.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+'</td><td class="r m">'+fxAvgRate.toFixed(4)+'</td><td class="r m b">'+fm(fxBookJpy)+'</td><td class="r m">'+fxCurRate.toFixed(4)+'</td><td class="r m b">'+fm(fxMvJpy)+'</td><td class="r">'+bg(fxPl)+'</td></tr></tbody></table>':'<div style="padding:20px;text-align:center;color:#64748b;font-size:12px">외화예수금 잔고가 없습니다. 편집 버튼으로 입력하세요.</div>'}</div>
...
function rFS(){
  ...
  '<div id="TC"><div class="pn" style="padding:18px;max-width:680px"><div style="text-align:center;margin-bottom:16px"><div style="font-size:16px;font-weight:700">손 익 계 산 서</div><div style="font-size:12px;color:#64748b">태성주식회사 · 제'+curFY()+'기 ('+fyRange(curFY()).start+' ~ '+fyRange(curFY()).end+') · 단위:엔</div></div>'+
  // ── Ⅰ 매출액 ──
  (function(){window._plN=0;return '';})()+
  '<div class="fr h"><span>'+_rn()+' 매출액</span><span class="m">'+fm(d.sales)+'</span></div>'+
  d.salesDetail.map(function(x){var a=D.accts.find(function(y){return y.c===x.c;});return '<div class="fr i"><span>'+(a?a.k:x.c)+' <span style="font-size:10px;color:#64748b">('+(a?a.n:'')+')</span></span><span class="m">'+fm(x.v)+'</span></div>';}).join('')+
  // ── Ⅱ 매출원가 ──
  ((d.cogs!==0||d.purchase!==0||d.invBegin!==0||d.invEnd!==0)?
    '<div class="fr h"><span>'+_rn()+' 매출원가</span><span class="m">'+fm(d.cogs)+'</span></div>'+
    '<div class="fr i"><span>기초재고</span><span class="m">'+fm(d.invBegin)+'</span></div>'+
    d.cogsDetail.map(function(x){var a=D.accts.find(function(y){return y.c===x.c;});return '<div class="fr i"><span>'+(a?a.k:x.c)+'</span><span class="m">'+fm(x.v)+'</span></div>';}).join('')+
    '<div class="fr i"><span>기말재고 (차감)</span><span class="m">△'+fm(d.invEnd)+'</span></div>'
    :'')+
  '<div class="fr b tl"><span>매출총이익</span><span class="m">'+fm(d.gross)+'</span></div><div style="height:8px"></div>'+
  '<div class="fr h"><span>'+_rn()+' 판매비와 일반관리비</span></div>'+
  ...
  (d.su!==0?'<div class="fr"><span>설립비</span><span class="m">'+fm(d.su)+'</span></div>':'')+
  '<div class="fr b tl" style="color:'+(d.opInc>=0?'#059669':'#dc2626')+'"><span>'+(d.opInc>=0?'영업이익':'영업손실')+'</span><span class="m">'+fm(d.opInc)+'</span></div><div style="height:8px"></div>'+
  '<div class="fr h"><span>'+_rn()+' 영업외수익</span></div>'+
  ...
  '<div class="fr h"><span>'+_rn()+' 영업외비용</span></div>'+
...
function rRpt(){
  var RL2=scopeFilter(D.real).sort(function(a,b){return String(a.dt||'').localeCompare(String(b.dt||''));});
  var _BI=scopeFilter(D.bkIn);
  var _BO=scopeFilter(D.bkOut);
  // 회기별 소계 (전기간 표시일 때)
  var _rsub={}; D.real.forEach(function(r){var f=recFY(r); if(!_rsub[f])_rsub[f]={n:0,tc:0,sa:0,gr:0,net:0}; _rsub[f].n++; _rsub[f].tc+=r.tc||0; _rsub[f].sa+=r.sa||0; _rsub[f].gr+=r.gr||0; _rsub[f].net+=r.net||0;});
  const c=calc();
  ...
  return '<div style="max-width:1100px" id="rptContent">'+
    '<div style="text-align:center;margin-bottom:20px"><div contenteditable="true" style="font-size:22px;font-weight:700;color:#1e3a5f">태성㈜ 자금운용보고서</div><div style="font-size:11px;color:#64748b;margin-top:2px">'+(recScope()==='all'?'전기간 누적 (제1기~제'+curFY()+'기)':'제'+curFY()+'기 '+fyRange(curFY()).start+' ~ '+fyRange(curFY()).end)+'</div><div style="font-size:13px;color:#64748b;margin-top:4px">'+rptDt()+' 기준</div></div>'+
    '<div class="no-print" style="display:flex;gap:8px;align-items:center;margin-bottom:10px;padding:8px 12px;background:#f8fafc;border:1px solid #e2e6ed;border-radius:6px;flex-wrap:wrap"><span style="font-size:12px;font-weight:600">보고 범위:</span>'+fySelector()+scopeSelector()+'<span style="font-size:11px;color:#64748b">현재 <b>'+scopeLabel()+'</b> 기준으로 3·4번 항목이 집계됩니다</span></div>'+
    ...
    '<tr><td>자본금</td><td class="r m">'+fm(acctBalAsOf('300',fyRange(curFY()).end))+'</td><td class="mu" style="font-size:10px">普通預金 입금</td></tr>'+
    ...
    // 3. 수익실현내역
    '<div style="display:flex;justify-content:space-between;align-items:center;margin:20px 0 8px"><div contenteditable="true" style="font-size:15px;font-weight:700;color:#1e3a5f">3. 수익실현내역 <span style="font-size:11px;color:#64748b;font-weight:400">('+scopeLabel()+' '+RL2.length+'건)</span></div><button class="bt gh no-print" style="font-size:10px" onclick="rptAddRow(\'수익실현\')">+ 행추가</button></div>'+
    ...
    (function(){
      var rr='';
      if(recScope()==='all'){
        Object.keys(_rsub).sort().forEach(function(f){
          var x=_rsub[f];
          rr+='<tr style="background:#f1f5f9;font-size:11px"><td colspan="4" class="r" style="font-weight:600">제'+f+'기 소계 ('+x.n+'건)</td>'+
              '<td colspan="2"></td><td class="r m">'+fm(x.tc)+'</td><td></td><td class="r m">'+fm(x.sa)+'</td>'+
              '<td class="r m">'+fm(x.gr)+'</td><td></td><td class="r">'+bg(x.net)+'</td>'+
              '<td class="r m">'+(x.tc?(x.net/x.tc*100).toFixed(2):'0.00')+'%</td></tr>';
        });
      }
    ...
    '<div class="pn"><div class="ph" style="color:#059669;font-size:12px"><span>입금 상세내역 ('+scopeLabel()+' '+_BI.length+'건)</span><button class="bt gh no-print" style="font-size:9px" onclick="rptAddRow(\'입금\')">+</button></div><table><thead><tr><th>일자</th><th>구분(내역)</th><th class="r">입금금액(엔)</th><th class="r">누적입금액(엔)</th></tr></thead><tbody>'+bkInRows+'</tbody></table></div>'+
    '<div class="pn"><div class="ph" style="color:#dc2626;font-size:12px"><span>출금 상세내역 ('+scopeLabel()+' '+_BO.length+'건)</span><button class="bt gh no-print" style="font-size:9px" onclick="rptAddRow(\'출금\')">+</button></div><table><thead><tr><th>일자</th><th>구분(내역)</th><th class="r">출금금액(엔)</th><th class="r">누적출금액(엔)</th></tr></thead><tbody>'+bkOutRows+'</tbody>'+
    '<tr class="t"><td colspan="2" class="r">'+scopeLabel()+' 출금계</td><td colspan="2" class="r m">'+fm(cO)+'</td></tr>'+
...
if(!D.customAccts)D.customAccts=[];
if(D._oiAccts){var _s={};OI_ACCTS=D._oiAccts.filter(function(c){return _s[c]?false:(_s[c]=true);});}
D.accts=mergeAccts(); // Always use fresh accounts
        saveD();
        // Restore settings
        if(backup.settings){
          SET=backup.settings;
...
function exportGLExcel(){
  ...
  html+='<p style="font-size:9pt;color:#666">第'+curFY()+'期　'+fyWareki()+'</p><br>';
...
<h1>損 益 計 算 書</h1>
<div class="sub">泰成株式会社<br>第${curFY()}期　${fyWareki()}<br>（単位：円）</div>
...
<tr class="sec"><td colspan="3">Ⅰ　売上高</td><td class="r">${fm(d.sales)}</td></tr>
${d.salesDetail.map(function(x){var a=D.accts.find(function(y){return y.c===x.c;});return '<tr><td>　'+(a?a.n:x.c)+'</td><td class="r">'+fm(x.v)+'</td><td></td><td></td></tr>';}).join('')}
${(d.cogs!==0||d.purchase!==0)?'<tr class="sec"><td colspan="3">Ⅱ　売上原価</td><td class="r">'+fm(d.cogs)+'</td></tr>'+'<tr><td>　期首棚卸高</td><td class="r">'+fm(d.invBegin)+'</td><td></td><td></td></tr>'+d.cogsDetail.map(function(x){var a=D.accts.find(function(y){return y.c===x.c;});return '<tr><td>　'+(a?a.n:x.c)+'</td><td class="r">'+fm(x.v)+'</td><td></td><td></td></tr>';}).join('')+'<tr><td>　期末棚卸高（控除）</td><td class="r">△'+fm(d.invEnd)+'</td><td></td><td></td></tr>':''}
<tr class="sub"><td colspan="3">売上総利益</td><td class="r b">${fm(d.gross)}</td></tr>
<tr class="gap"><td colspan="4"></td></tr>

<tr class="sec"><td colspan="4">Ⅲ　販売費及び一般管理費</td></tr>
${(function(){var excl=['537','540','541','542','543','544','545','546','550','551','552','553','560','561','562','563','564','565'];var r='';D.accts.filter(function(ac){return ac.g==='비용'&&excl.indexOf(ac.c)<0;}).forEach(function(ac){var b=acctBalFY(ac.c);if(b>0)r+='<tr><td>　'+(ac.n||ac.k)+'</td><td class="r">'+fm(b)+'</td><td></td><td></td></tr>';});return r;})()}
<tr class="sub"><td>　販管費合計</td><td></td><td class="r b">${fm(d.sgaT)}</td><td></td></tr>
<tr><td>　創立費</td><td></td><td class="r">${fm(d.su)}</td><td></td></tr>
<tr class="sub"><td>${d.opInc>=0?'営業利益':'営業損失'}</td><td></td><td></td><td class="r b" style="color:${d.opInc>=0?'#27ae60':'#c0392b'}">${fm(d.opInc)}</td></tr>
<tr class="gap"><td colspan="4"></td></tr>

<tr class="sec"><td colspan="4">Ⅳ　営業外収益</td></tr>
${(function(){var r='';D.accts.filter(function(ac){return ac.g==='수익'&&ac.c!=='414';}).forEach(function(ac){var b=acctBalFY(ac.c);if(b>0)r+='<tr><td>　'+(ac.n||ac.k)+'</td><td class="r">'+fm(b)+'</td><td></td><td></td></tr>';});if(d.evalGain>0)r+='<tr><td>　有価証券評価益（未実現）</td><td class="r">'+fm(d.evalGain)+'</td><td></td><td></td></tr><tr><td colspan="4" class="note">　　※保有銘柄の時価基準により自動反映</td></tr>';return r;})()}
<tr class="sub"><td>　営業外収益合計</td><td></td><td></td><td class="r b">${fm(d.noiT)}</td></tr>
<tr class="gap"><td colspan="4"></td></tr>

<tr class="sec"><td colspan="4">Ⅴ　営業外費用</td></tr>
...
<tr class="gap"><td colspan="4"></td></tr>
<tr class="sec"><td colspan="3">Ⅵ　法人税等</td><td class="r">${fm(d.ct)}</td></tr>
<tr class="gap"><td colspan="4"></td></tr>
...
<div class="stamp"><div class="s1">決裁</div><div class="s2">本人専決</div></div>
<h1>貸 借 対 照 表</h1>
<div class="sub">泰成株式会社<br>第${curFY()}期　${warekiDate(d.bsEnd)}現在<br>（単位：円）</div>
...
${d.fxDep!==0?'<tr><td>　外貨証券預り金</td><td class="r">'+fm(d.fxDep)+'</td><td></td><td></td></tr>':''}
<tr class="sub"><td>　現金・預金計</td><td></td><td class="r b">${fm(d.cashT)}</td><td></td></tr>
<tr class="sub"><td>　有価証券（帳簿価額）</td><td></td><td class="r b">${fm(d.secForBS)}</td><td></td></tr>
${(isCurFYOpen()&&d.secMV!==d.secForBS)?'<tr><td colspan="4" class="note">　　※時価 '+fm(d.secMV)+'（未実現評価損益 '+fys(d.secMV-d.secForBS)+'）洗替方式により決算時に評価</td></tr>':''}
${(function(){var rows='';D.accts.filter(function(ac){return ac.g==='자산'&&['110','191','130','192'].indexOf(ac.c)<0;}).forEach(function(ac){var b=acctBalAsOf(ac.c,d.bsEnd);if(b!==0)rows+='<tr><td>　'+ac.n+'</td><td class="r">'+fm(b)+'</td><td></td><td></td></tr>';});return rows;})()}
...
${(function(){var rows='';D.accts.filter(function(ac){return ac.g==='부채';}).forEach(function(ac){var b=acctBalAsOf(ac.c,d.bsEnd);if(b!==0)rows+='<tr><td>　'+ac.n+'</td><td class="r">'+fm(b)+'</td><td></td><td></td></tr>';});return rows||'<tr><td colspan="4" class="note">　　負債なし</td></tr>';})()}
<tr class="total"><td>負債合計</td><td></td><td></td><td class="r">${fm(d.totL)}</td></tr>
<tr class="gap"><td colspan="4"></td></tr>

<tr class="sec"><td colspan="4">【純資産の部】</td></tr>
${(function(){var rows='';D.accts.filter(function(ac){return ac.g==='순자산';}).forEach(function(ac){var b=acctBalAsOf(ac.c,d.bsEnd);if(b!==0)rows+='<tr><td>　'+ac.n+'</td><td class="r">'+fm(b)+'</td><td></td><td></td></tr>';});return rows;})()}
${d.retainedBal!==0?'<tr><td>　前期繰越利益剰余金</td><td class="r">'+fm(d.retainedBal)+'</td><td></td><td></td></tr>':''}
<tr><td>　当期純利益</td><td class="r">${fm(d.eqNI)}</td><td></td><td></td></tr>
...
<div class="footer">泰成株式会社 財務管理システム ｜ 出力日: ${dateStr} ｜ 第${curFY()}期 ${fyWareki()}</div>
</body></html>`;
...
function rTaxSummary(){
  var A=taxAggregate(), f=A.fy, R=fyRange(f);
  var pct=(A.ratio*100).toFixed(2);
  var deduct = A.fullDeduct ? A.inTax : A.ippan;   // 공제 가능 매입세액
  var payable = A.outTax - deduct;                  // 양수=납부 / 음수=환급
  var h='';

  h+='<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:11px">';
  h+='<b>📋 일반과세 (本則課税) · 세포함 경리 (税込経理方式)</b> — 세리사 방식과 통일';
  h+='<br>자본금 1,000만엔 → 소비세법 §12-2에 의해 설립 제1기부터 과세사업자';
  h+='<br>제'+f+'기 '+R.start+' ~ '+R.end+' · 신고기한: 결산일 익일부터 2개월 이내';
  h+='</div>';

  // ── 課税売上割合 ──
  h+='<div class="pn" style="padding:14px;margin-bottom:14px"><div style="font-size:13px;font-weight:700;margin-bottom:8px">📊 課税売上割合</div>';
  h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">';
  h+='<div style="text-align:center;padding:10px;background:#f8fafc;border-radius:6px"><div style="font-size:10px;color:#64748b">분자 (課税売上+輸出免税)</div><div style="font-size:15px;font-weight:700">'+fm(A.num)+'</div></div>';
  h+='<div style="text-align:center;padding:10px;background:#f8fafc;border-radius:6px"><div style="font-size:10px;color:#64748b">분모 (+非課税売上)</div><div style="font-size:15px;font-weight:700">'+fm(A.den)+'</div></div>';
  h+='<div style="text-align:center;padding:10px;background:'+(A.fullDeduct?'#d1fae5':'#fef2f2')+';border-radius:6px"><div style="font-size:10px;color:#64748b">割合</div><div style="font-size:18px;font-weight:700;color:'+(A.fullDeduct?'#059669':'#dc2626')+'">'+pct+'%</div></div>';
  h+='</div>';
  h+='<table style="font-size:11px"><tbody>';
  h+='<tr><td>課税売上 (10%·8%·輸出免税0%)</td><td class="r m">'+fm(A.taxSale)+'</td><td class="mu" style="font-size:10px">분자·분모</td></tr>';
  h+='<tr class="a"><td>非課税売上 (수취이자 등)</td><td class="r m">'+fm(A.nonTaxSale)+'</td><td class="mu" style="font-size:10px">분모만</td></tr>';
  h+='<tr><td>유가증권 양도대가 '+fm(A.secSale)+' × 5%</td><td class="r m">'+fm(A.secDen)+'</td><td class="mu" style="font-size:10px">시행령 §48③ (수익실현 기준)</td></tr>';
  h+='<tr class="a"><td colspan="3" class="mu" style="font-size:10px">※ 403 有価証券売却益은 분모에 넣지 않습니다 — 산입액은 매각익이 아니라 <b>양도대가의 5%</b>이므로 위 행과 이중계상됩니다</td></tr>';
  h+='</tbody></table>';
  h+='<div style="font-size:10px;color:#64748b;margin-top:6px">'+(A.den===0?'※ 課税売上·非課税売上 모두 0 → 割合 산정 불가':(A.fullDeduct?'※ 95% 이상 → 매입세액 <b>전액공제</b>':'※ 95% 미만 → 個別対応方式 또는 一括比例配分方式으로 안분 필요'))+'</div>';
  h+='</div>';
  ...
  function tbl(title,list,color,B){
    ...
    if(i===0) r+='<tr><td colspan="5" style="text-align:center;color:#94a3b8;padding:12px">해당 없음</td></tr>';
    else r+='<tr style="font-weight:700;background:#f1f5f9"><td colspan="2">합계</td><td class="r m">'+fm(tA)+'</td><td class="r m">'+fm(tT)+'</td><td class="r">'+tC+'</td></tr>';
  ...
  h+='<div style="background:#fff;border:1px solid #e2e6ed;border-radius:8px;padding:12px;text-align:center;border-top:3px solid #d97706"><div style="font-size:10px;color:#d97706;font-weight:600">매출세액 (売上税額)</div><div style="font-size:16px;font-weight:700;margin-top:4px">'+fm(A.outTax)+'</div></div>';
  h+='<div style="background:#fff;border:1px solid #e2e6ed;border-radius:8px;padding:12px;text-align:center;border-top:3px solid #2563eb"><div style="font-size:10px;color:#2563eb;font-weight:600">공제대상 매입세액</div><div style="font-size:16px;font-weight:700;margin-top:4px">'+fm(deduct)+'</div><div style="font-size:9px;color:#64748b">발생 '+fm(A.inTax)+' × '+(A.fullDeduct?'전액':pct+'%')+'</div></div>';
  var isRef=payable<0;
  h+='<div style="background:#fff;border:1px solid #e2e6ed;border-radius:8px;padding:12px;text-align:center;border-top:3px solid '+(isRef?'#059669':'#dc2626')+'"><div style="font-size:10px;color:'+(isRef?'#059669':'#dc2626')+';font-weight:600">'+(isRef?'환급세액 (還付)':'납부세액 (納付)')+'</div><div style="font-size:16px;font-weight:700;margin-top:4px;color:'+(isRef?'#059669':'#dc2626')+'">'+fm(Math.abs(payable))+'</div></div>';
  h+='</div>';
  ...
  // ── 税区分 정규화 ──
  var unnorm=D.journals.filter(function(j){return j.taxCls&&TAXCLS_ALL.indexOf(j.taxCls)<0;}).length;
  var noCls=D.journals.filter(function(j){return !j.taxCls;}).length;
  h+='<div class="pn" style="padding:14px;margin-bottom:14px"><div style="font-size:13px;font-weight:700;margin-bottom:6px">🔧 税区分 정규화</div>';
  h+='<div style="font-size:11px;color:#64748b;margin-bottom:8px">구 표기(과세10%·불과세·課税10% 등)를 소비세법 11종 표준 구분으로 일괄 변환합니다. 계정별 기본 구분(ACCT_TAX_DEFAULT) 기준이며 <b>금액·계정은 변경하지 않습니다</b>.</div>';
  h+='<div style="font-size:11px;margin-bottom:8px">비표준 표기 <b>'+unnorm+'건</b> · 미지정 <b>'+noCls+'건</b> (전체 '+D.journals.length+'건)</div>';
  h+='<button class="bt" onclick="previewTaxNorm()" style="background:#2563eb">🔍 변환 미리보기</button> ';
  ...
  h+='• <b>415 売上高-輸出免税</b>는 분자·분모 모두 산입되어 割合을 올립니다 → 환급에 유리<br>';
  h+='• <b>416 売上高-国外不課税</b>는 분자·분모 모두 불산입 → 割合에 영향 없음</div>';
  return h;
}
...
function normTaxCls(j,side,code){
  var cur=j.taxCls||'';
  var fam=(side==='buy'?TAXCLS_BUY:TAXCLS_SELL).concat(['対象外']);
  ...
  if(def) return def;
  if(/^(과세|課税)10/.test(cur)) return side==='buy'?'課税仕入10%':'課税売上10%';
  if(/8%/.test(cur)) return side==='buy'?'課税仕入8%':'課税売上8%';
  if(/비과세|非課税/.test(cur)) return side==='buy'?'非課税':'非課税売上';
  if(/불과세|不課税/.test(cur)) return side==='buy'?'不課税':'不課税売上';
  return '対象外';
}
...
function taxNormPlan(){
  ...
  D.journals.forEach(function(j){
    ...
    if(isExp[j.dr]){side='buy';code=j.dr;}
    else if(isExp[j.cr]){side='buy';code=j.cr;}
    else if(isRev[j.cr]){side='sell';code=j.cr;}
    else if(isRev[j.dr]){side='sell';code=j.dr;}
    var nc = side ? normTaxCls(j,side,code) : '対象外';   // BS↔BS 전표는 対象外
    if((j.taxCls||'')!==nc) plan.push({id:j.id,no:j.no,acct:code||'',desc:(j.desc||'').slice(0,30),amt:j.amt,from:(j.taxCls||'(없음)'),to:nc});
  });
  return plan;
}
...
function previewTaxNorm(){
  ...
  if(!el) return;
  if(!plan.length){ el.innerHTML='<div style="margin-top:10px;padding:10px;background:#d1fae5;border-radius:6px;font-size:11px">✅ 이미 전부 정규화되어 있습니다.</div>'; return; }
  var agg={};
  plan.forEach(function(p){var k=p.from+' → '+p.to; agg[k]=(agg[k]||0)+1;});
  var h='<div style="margin-top:10px;padding:10px;background:#f8fafc;border:1px solid #e2e6ed;border-radius:6px">';
  h+='<div style="font-size:12px;font-weight:700;margin-bottom:6px">변환 대상 '+plan.length+'건 / 전체 '+D.journals.length+'건</div>';
  h+='<table style="font-size:11px"><thead><tr><th>변환</th><th class="r">건수</th></tr></thead><tbody>';
  Object.keys(agg).sort(function(a,b){return agg[b]-agg[a];}).forEach(function(k,i){
...
function runTaxNorm(){
  var plan=taxNormPlan();
  if(!plan.length) return toast('이미 전부 정규화되어 있습니다','info');
  if(!confirm('税区分 '+plan.length+'건을 표준 구분으로 변환합니다.\n금액·계정은 변경되지 않습니다.\n\n계속하시겠습니까?')) return;
  try{ localStorage.setItem('taesung_taxcls_BACKUP', JSON.stringify(D.journals.map(function(j){return {id:j.id,taxCls:j.taxCls||''};}))); }
  catch(e){ return toast('백업 실패 — 중단합니다: '+e.message,'error'); }
  if(!localStorage.getItem('taesung_taxcls_BACKUP')) return toast('백업 검증 실패 — 중단합니다','error');
  var m={}; plan.forEach(function(p){m[p.id]=p.to;});
  var n=0; D.journals.forEach(function(j){ if(m[j.id]!==undefined){ j.taxCls=m[j.id]; n++; } });
  saveD();
...
function genTaxSettlement(){
  var _stlEnd=fyRange(curFY()).end;                      // 당기 결산일 (회기 연동)
  var _stlMD=parseInt(_stlEnd.slice(5,7))+'/'+parseInt(_stlEnd.slice(8,10));
  var _stlFY=curFY();
  ...
  if(netAmt>0){
    D.journals.push({id:nid(),dt:_stlMD,no:genSlipNo(_stlEnd,'211','154'),desc:'소비세 상계 (가수↔가지급) [소비세정산]',dr:'211',cr:'154',amt:netAmt,edt:_stlEnd,pdt:_stlEnd,cur:'JPY',exp:'',vendor:'',taxCls:'',fy:_stlFY});
    count++;
  }
  // Step 2: remainder
  ...
  if(remainder>0){
    if(isRefund){
      ...
      D.journals.push({id:nid(),dt:_stlMD,no:genSlipNo(_stlEnd,'122','154'),desc:'소비세 환급액 [소비세정산]',dr:'122',cr:'154',amt:remainder,edt:_stlEnd,pdt:_stlEnd,cur:'JPY',exp:'',vendor:'',taxCls:''});
    } else {
      // 납부: DR 211(가수소비세) / CR 206(미지급소비세)
      D.journals.push({id:nid(),dt:_stlMD,no:genSlipNo(_stlEnd,'211','206'),desc:'소비세 납부액 [소비세정산]',dr:'211',cr:'206',amt:remainder,edt:_stlEnd,pdt:_stlEnd,cur:'JPY',exp:'',vendor:'',taxCls:''});
    }
    count++;
  }
  saveD();
...
function rBSTab(){
  ...
  var _e=d.bsEnd;
  var extraAssets='';
  D.accts.filter(function(ac){return ac.g==='자산'&&['110','191','130','192'].indexOf(ac.c)<0;}).forEach(function(ac){var b=acctBalAsOf(ac.c,_e);if(b!==0)extraAssets+='<div class="fr"><span>'+ac.k+'</span><span class="m">'+fm(b)+'</span></div>';});
  // Dynamic liability items
  var liabItems='';
  D.accts.filter(function(ac){return ac.g==='부채';}).forEach(function(ac){var b=acctBalAsOf(ac.c,_e);if(b!==0)liabItems+='<div class="fr"><span>'+ac.k+'</span><span class="m">'+fm(b)+'</span></div>';});
  // Dynamic equity items
  var eqItems='';
  D.accts.filter(function(ac){return ac.g==='순자산';}).forEach(function(ac){var b=acctBalAsOf(ac.c,_e);if(b!==0)eqItems+='<div class="fr"><span>'+ac.k+'</span><span class="m">'+fm(b)+'</span></div>';});
  if(d.retainedBal!==0)eqItems+='<div class="fr"><span>전기이월이익잉여금</span><span class="m">'+fm(d.retainedBal)+'</span></div>';
  ...
  (d.fxDep!==0?'<div class="fr"><span>외화증권예수금</span><span class="m">'+fm(d.fxDep)+'</span></div>':'')+
  ...
  (isCurFYOpen()?'<div class="fr" style="font-size:10px;color:#64748b"><span>　※시가평가 '+fm(d.secMV)+' (미실현 '+fys(d.secMV-d.secBookVal)+')</span><span></span></div>':'<div class="fr" style="font-size:10px;color:#64748b"><span>　※결산확정 장부가 (과거 회기)</span><span></span></div>')+
  ...
  '<div class="ib" style="font-size:10px">💡 제'+curFY()+'기 결산일('+d.bsEnd+') 시점 누적잔액 기준. 차대차액 '+fm(d.totA-d.totL-d.totE)+'</div>';
}
